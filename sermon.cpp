...
void draw_serial_output() {
    ...
    // Calculate scrollbar thumb position (Row 3 to 21)
    int thumb_y = 3;
    if (max_scroll > 0) {
        thumb_y = 3 + ((long)scroll_y * 18) / max_scroll;
    }
    ...
    for (int r = 3; r <= 21; r++) {
        ...
        if (line_idx < total_serial_lines) {
            ...
            // 3. ERASER LOOP: Blank out the rest of the line to clear old ghost text
            for (int c = 6 + len; c <= 78; c++) {
                set_char(r, c, ' ', bg);
            }
        } else {
            // ERASER LOOP: If the row is totally empty, blank out the gutter and text area
            for (int c = 1; c <= 4; c++) set_char(r, c, ' ', bg); 
            for (int c1 = 6; c1 <= 78; c1++) set_char(r, c1, ' ', bg);
        }
        
        // Render the Dynamic Scrollbar (Col 79)
        if (total_serial_lines > max_vis_lines) {
            unsigned char sc_char = (setting_ansi_mode) ? 176 : ':'; // Scroll track
            if (r == thumb_y) sc_char = (setting_ansi_mode) ? 219 : '#'; // Scroll thumb
            set_char(r, 78, sc_char, bg);
        } else {
            // No scrollbar needed, just draw the standard border
    ...
    if (show_settings) {
        ...
        if (mouse_y == 1 && mouse_x >= 76 && mouse_x <= 78) running = 0; 
            ...
            // 2. Text Box Hitbox
                ...
                // NEW: Shift-Click selection support!
            ...
            // 3. Send Button Hitbox
                ...
                if (input_buffer[0] != '\0') {
                    ...
                    // FIXED: Use standard theme color instead of highlight
            ...
            // Scrollbar Hitbox (Col 79)
            if (mouse_x == 79 && total_serial_lines > max_vis_lines) {
                int max_scroll = total_serial_lines - max_vis_lines;
                // Jump the scroll position to exactly where the mouse clicked
                int new_scroll = ((mouse_y - 3) * max_scroll) / 18; 
